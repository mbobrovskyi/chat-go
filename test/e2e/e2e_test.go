...
var _ = ginkgo.Describe("Chat", func() {
	...
	ginkgo.Context("update chat endpoint", func() {
		ginkgo.It("should return valid response for updating group chat", func() {
			...
			updatedChat := util.UpdateChat(client, chatURL, util.AdminToken, updateChatRequest, http.StatusOK)
		...
		ginkgo.It("should return forbidden error when unauthorized user tries to update chat", func() {
			...
			updatedChat := util.UpdateChat(client, chatURL, util.RandomID(), updateChatRequest, http.StatusForbidden)
			gomega.Expect(updatedChat).To(gomega.BeNil())
		})
	})
})
